package WebTools::DataSvc::TransferQueueStats;
...
{
  my ($self, $attr) = @_;
  croak qq|Either source or destination must be specified!| 
  ...
  my $params = '';
  for my $tag (qw/from to/)
  {
    $params .= qq|$tag=$attr->{$tag}| if defined $attr->{$tag};
  }

  my $content = $self->content({ cmd => q|transferqueuestats|, options => $params });
...
__END__
package main;
my $obj = new WebTools::DataSvc::TransferQueueStats({ instance => q|debug| });
