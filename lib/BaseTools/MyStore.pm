...
sub new
{
  ...
  my $dbfile = $attr->{dbfile} or carp qq|dbfile not specified|;
  -e $dbfile or carp qq|$dbfile not found!|;
...
BaseTools::MyStore is a simple OO wrapper over C<Storable> that can manipulate
the underlying data structure, usually a hash reference, using C<add()>, C<remove()>,
C<exists()>, C<get()> and other related methods.
...
$Id: MyStore.pm,v 1.0 2008/06/17 00:03:19 sarkar Exp $
